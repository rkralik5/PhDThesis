...
\note{Is this derivation too trivial to mention in a thesis? Should I just mention the results?}
...
GENIE \gls{MC} generator only considers tree-level \gls{SM} \gls{nuone} interactions \cite{NOVA-doc-56383} as were described in Sec.~\ref{sec:MeasuringNuMM}. However, when focusing on \gls{nuone} interaction, we have to account for higher orders of the perturbative expansion of the interaction Lagrangian and apply radiative corrections. These are contained in two adjustments to the tree-level \gls{SM} cross section for the \gls{nuone} interaction \cite{MinervaNuoneFluxConstraint2019.pdf}. First, the coupling constants change \cite{NuoneRadCorrConstants2013.pdf}
\begin{equation}
g_1^{\nu_e}\rightarrow 0.7276,\; \; g_1^{\nu_\mu}\rightarrow -0.2730,\; \;  g_2\rightarrow 0.2334.
\end{equation}
Second, the cross section equation has additional terms, which for one-loop correction look like \cite{NuoneRadCorrEquation1984.pdf}
\begin{equation}
...
\end{equation}
and\footnote{There is technically also a third correction term $X_3$, which is however negligible for $E_\nu\sim\unit{GeV}$.}
\begin{equation}
...
Practically, radiative corrections are applied in the form of weights, where each event is weighted by a ratio
\begin{equation}
...
Analogically to the radiative correction weight it is possible to create a neutrino magnetic moment weight as a ratio between the neutrino magnetic moment and the GENIE \gls{SM} differential cross sections. This is possible thanks to the theoretically well understood properties of the \gls{nuone} interaction, as described in Sec.~\ref{sec:MeasuringNuMM}. This effectively emulates the neutrino magnetic moment simulation without requiring any additional processing power. Therefore out signal sample can be created from the same events as a true \gls{nuone} sample, with the magnetic moment weight applied. The weight has a form:
\begin{equation}
