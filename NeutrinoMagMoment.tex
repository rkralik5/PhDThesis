...
[Progression review 3] Measuring the neutrino magnetic moment would give us a strong hint towards the new physics beyond the standard model. If neutrinos are Dirac particles, their magnetic moment would be too small to measure in any current experiments (in most theories beyond the standard model). However if neutrinos are Majorana particles, current experiments including NOvA and DUNE could be able to see its effect. The most sensitive measurement of neutrino magnetic moment is the study of neutrinos elastically scattering off of electrons. If neutrino would have magnetic moment, it would add (without interference) a term to the neutrino-on-electron elastic scattering cross section, which is quadratically dependent on the size of the effective magnetic moment and linearly dependent on the inverse of the electron recoil energy (see fig.1). The effective magnetic moment is related to the matrix elements of the dipole magnetic moment via the neutrino oscillation parameters and is different (but interconnected) for different neutrino flavours [3]. The purity of the NuMI beam and the use of muon neutrinos and antineutrinos give NOvA a possibility to provide world leading measurement of the muon neutrino effective magnetic moment [8].

[Thesis readiness review] Neutrinos in theories beyond the Standard Model can obtain a measurable neutrino magnetic moment, which would manifest in the NOvA Near Detector as an excess of neutrino-on-electron elastics scattering interactions in low electron recoil energies. Measuring an enhanced neutrino magnetic moment would point towards the right theory beyond the Standard Model, inform us about the necessary energy scale of neutrino mass generation, or point towards neutrinos being Majorana particles. 
...
NOvA benefits from a near pure source of muon neutrinos and antineutrinos and should be able to provide a world leading limit on the muon neutrino magnetic moment.

%%% HOW - neutrino on electron elastic scattering
...
In the future, this analysis can be improved for example by improving the event selection, creating additional background samples, by including antineutrino events, or by using a fit to the energy distribution, similarly to the \gls{LDM} analysis.
...
We are using the standard \gls{NOvA} simulation and reconstruction, specifically the same tools that were used in analyses that use Prod5.1 (which are?)

We are using data collected from the start of \gls{NOvA} data taking on 22$^{\textsf{nd}}$ of August 2014, until 3$^{\textsf{rd}}$ of March 2021. \gls{NOvA} collected further data from that point, those are however still being processed and are not available at the time of writing this thesis. The exposure of the data sample is approximately $13.8\times 10^{20}$\gls{POT}.

The NSI paper claims exposure of 13.6E20 POT from February 6 2014 to March 20 2020. - this is the 2020 exposure from the paper.

Periods used in prod 5.1
ND run first: 10391 - 
ND run last:  14010
...
\note{Do I have to discuss these different signal definitions?}
\begin{table}[!ht]
\centering
\caption{Overview of signal and background definitions.}
\def\arraystretch{1.4}
\begin{tabular}{p{.25\textwidth}p{.7\textwidth}}
Signal                   & \gls{nuone} \&\& \texttt{TrueVtxContained}\\
$\nu$-on-e background    & \gls{nuone}\\
$\nu_e$CC MEC background & !(\texttt{el scattering} \&\& \texttt{el In Fin State}) \&\& (\gls{CC} \&\& $\nu_e$ \&\& \gls{MEC})\\
Other background         & !(\texttt{el scattering} \&\& \texttt{el In Fin State}) \&\& !(\gls{CC} \&\& $\nu_e$ \&\& \gls{MEC})
\end{tabular}
...
again Literally taken from the 3 flavour's nue appearance low energy selection
...
\todo{Mention that I added a preliminary $E\theta^2<0.04$ cut}
...
\note{Already applying the cut here to help reduce the events for TMVA. This cut is based purely on reco quality (we don't trust events below 0.5GeV, especially not for the CVN nuone ID variables0}
...
%%%                           FITTING                             %%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Fitting and hypothesis testing, parameter estimation}\label{sec:NuMMFitting}
