...
%Null hypothesis
\subsection{Hypothesis Testing and Confidence Limits}
...
In case of the null hypothesis, $H_0$, the expected number of events is simply the expected number of the \gls{SM} background. For the used data exposure of $1.38\times 10^{21}$ \gls{POT}, this is
\begin{equation}
N_{H_0}=N_B=700.33 \pm 26.46\left(\textsf{stat.}\right) ^{+72.48}_{-62.99}\left(\textsf{syst.}\right)
\end{equation}

For the alternative hypothesis, $H_I$, the expected number of events includes the signal events, which depend on the size of the neutrino magnetic moment and can be expressed as:
\begin{equation}\label{eq:NuMMExpectedNumberOfSignalEvents}
N_{H_I}=N_B+N_S=N_B+56.80\times\left(\frac{\mu_\nu}{10^{-9}\mu_B}\right)^2.
\end{equation}
...
The minimum log likelihood for our null hypothesis is $1.02964$, which corresponds to the double sided p-value of $0.31$, or one-sided p-value of $0.16$ \note{To be honest, I am actually not sure if this is true or not. Calculating this using ROOT::Math::chisquared\_cdf\_c(1.02964,1) gives 0.31, and according to the documentation this should just be the upper tail of the chi squared distribution, which is I guess what I want. However, this website claims that this is the double sided p value and that I probably want one sided p value... https://www.graphpad.com/quickcalcs/pValue2/}, which is larger than $0.1$, therefore we are not able to reject the null hypothesis. Therefore we are putting a limit on the neutrino magnetic moment parameter of $\mu_\nu<1.91\times10^{-9}\mu_B$ at $90\%$ C.L., as shown in Fig.~\ref{fig:NuMMResults_ChisqLimit}. This limit is less strict than the current world-leading limit, or than the previous limit in \gls{NOvA}. However, there are several possible improvement that could improve our analysis. We can also estimate the systematic uncertainty on the best fit point by taking the $1\sigma$ range $\left(\chi^2=1\right)$ of the full systematic uncertainty distribution from Fig.~\ref{fig:NuMMResults_ChisqLimit}. The $1\sigma$ range for all systematics is $\left(0.14,1.63\right)\times 10^{-9}\mu_B$, which is equivalent to the best fit point having an uncertainty $\mu_{\nu,\acrshort{BF}}=\left(1.13^{+0.50}_{-0.99}\right)\times 10^{-9}\mu_B$.

\begin{figure}[hbtp]
\centering
\includegraphics[width=\textwidth]{Plots/NuMM/NuMMLimit_FinalMoneyPlot.pdf}
\caption[Fit results]{Results of the fit of the prediction to the observed data, with the neutrino magnetic moment as the sole fit parameter. We are profiling over the full range of systematic uncertainties as explained in text. \todo{Finish this caption}}
