...
We analysed the selection of \gls{nuone} events in the \gls{NOvA} \gls{ND} searching for a possible signal of the effective muon neutrino magnetic moment. We found the \gls{BF} point of $\mu_{\nu_\mu,\acrshort{BF}}=\left(1.13^{+0.50}_{-0.99}\right)\times 10^{-9}\mu_B$. We failed to reject the null hypothesis that the observed events can be described by \gls{SM} interactions only a placed an upper limit on the value of the effective muon neutrino magnetic moment of $\mu_{\nu_\mu}<19.1\times10^{-10}\mu_B$ at $90\%$ \gls{CL}.
...
The background prediction is dominated by systematic uncertainties, especially by the uncertainties related to the neutrino flux prediction. However, the number of neutrino magnetic moment signal events is much smaller than the \gls{SM} background and therefore there is a significant statistical uncertainty as well.

Our $90\%$ \gls{CL} limit on the effective muon neutrino magnetic moment is less strict than the current world-leading \cite{PDG.pdf} limit for muon neutrinos of $\mu_{\nu_\mu}<6.8\times 10^{-10}\mu_B$  at $\unit[90]{\%}$ \gls{CL} \cite{LSNDLimits2001.pdf}. It is also less strict than the previous result from \gls{NOvA} of $\mu_{\nu_\mu}<15.8\times 10^{-10}\mu_B$ at $\unit[90]{\%}$ \gls{CL} \cite{nuMM-thesis-biaow.pdf}, which however does not take into account the effect of systematic uncertainties.

%Improvements in statistics, adding RHC data and more data to follow
There are several possibilities for improvements in future iterations of the analysis. The easiest improvement lies in increasing the number of events. The \gls{NOvA} experiment is scheduled to run until ... which would bring in more than double the \gls{FHC} data than were used in the current analysis. Additionally, it is possible to include the \gls{RHC} data as well, either by simply including them into the current data, or by analysing them separately. A separate analysis could mean a disentanglement of some of the backgrounds and systematic uncertainties that are different between the neutrino and antineutrinos, since the effect of the neutrino magnetic moment should be exactly the same for neutrinos as for antineutrinos.

% Improved reconstruction, simulation and calibration - Test Beam plug
Further improvements could come from an improved detector simulation, calibration, and event reconstruction, especially related to single electron events. These improvements will be made possible by analysing the data from the Test Beam detector and by the improvements in calibration that come from the Test Beam detector calibration, as mentioned in Sec...

%Improved event selection - going below 0.5 GeV, creating control samples
Another improvement lies in improving the event selection, where we were forced to place a low energy cut of $\unit[0.5]{GeV}$ as we did not have enough time to properly investigate the low energy backgrounds and the effect of systematic uncertainties at low energy \gls{nuone} events. In the future it might be possible to extend this low energy cut-off to lower energies and therefore significantly improve our signal-to-background ratio. As can be seen in Fig.~\ref{fig:NuMMBackgroundDecomposition}, there are significant background that rise at low energies with a similar shape to the signal, therefore it would be imperative to properly study these background before extending the low energy cut to lower energies.

% Control samples
It is also possible to extend the statistical analysis by including one or more control samples. Control samples would be regions containing a specific type of background, while not including any (or almost any) signal events. These control regions could then be fit simultaneously to the signal region, providing a constraint on the backgrounds as well as on their systematic uncertainties.

%Improving the fitting - doing an energy dependent fit
There is also a possibility to extend the fitting procedure from a single bin fit to multi-bin fitting, likely with bins distributed in the reconstructed calorimetric energy of the leading shower. This possibility could use a technique called \textit{template fitting}, which assumes that the prediction is capable of describing the shape of the signal sample, the \gls{SM} \gls{nuone} sample, and the other \gls{SM} background sample, while leaving their individual normalizations floating free in the fit. This reduced the amount of free parameters in the fit while preserving the benefit of the knowledge of the difference between the shapes of the signal and background samples. Especially since the neutrino magnetic moment signal has a starkly different shape in the reconstructed calorimetric energy of the leading shower, with a steep rise towards the lower energies. However, as mentioned previously, the energy dependence of the \gls{SM} background can resemble the energy dependence of the signal. Specifically, the energy dependent contribution of the \gls{NC} interactions with a $\pi^0$ in the final state make it important to consider these uncertainties for an energy dependent analysis.

%Improving systematic uncertainties - utilising more results from NA61 into PPFX, especially NOvA replica target
There is also a likely large improvement to the systematic uncertainties, especially to the largest systematic uncertainty of the neutrino beam prediction coming from the new results from NA61 and EMPHATIC experiments, as outlined in Sec....
