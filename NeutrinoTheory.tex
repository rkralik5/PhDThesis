...
Currently the three neutrino paradigm is well established.
The existence of three neutrino mass states implies two independent neutrino mass splittings. The magnitude of both is well measured, as well as the size of two mixing angles, $\theta_{12}$ and $\theta_{13}$, and for $\theta_{23}$ we know it is close to the maximum mixing value $45^{\circ}$. However, there are three main questions yet to be determined. Is the sign of the larger neutrino mass splitting positive or negative (is neutrino hierarchy normal or inverted)? Is $\theta_{23}<45^{\circ}$ or $\theta_{23}>45^{\circ}$ (what is the octant of $\theta_{23})$? What is the value of $\delta_{CP}$ (is there \gls{CP} symmetry violation in the neutrino sector)? There is currently an intense experimental program ongoing to measure this values \cite{SnowmassNeutrinoFrontierReport.pdf,PDG.pdf}.
...
The existence of oscillations between more than one neutrino mass states requires neutrinos to have non-zero mass. This is to-date the only experimentally proved evidence of physics \gls{BSM}.
