...
NOvA employs a convolutional neural network (CNN) based on the GoogLeNet \cite{GoogLeNetArchitecture.pdf} architecture named Convolution Visual Network (CVN) \cite{CVN.pdf}, which uses slice hits to classify interactions into one of the four categories: $\nu_e$, $\nu_\mu$, $\nu_\tau$, or NC. The same architecture, but applied to the Fuzzy-K prongs and called ProngCVN \cite{PhihasNOvAThesis_ProngCVN.pdf}, is used to identify the individual prongs based on what particles they most likely correspond to. A special ML algorithm for identifying muons, based on a Boosted Decision Tree (BDT) with inputs from the Kalman track, is called Reconstructed Muon Identifier (ReMId)~\cite{RaddatzNOvAThesis_KalmanTracks.pdf}.
