...
To get a prediction of any possible signal events or their background in the NOvA detectors, we use a series of simulations, often tuned or corrected from both internal and external measurements \cite{NOvASimulationOld-Fluka.pdf}. The simulation chain can be divided into four parts: simulation of the neutrino beam, simulation of neutrino interactions within the NOvA detectors, simulation of cosmic particles interacting in the NOvA detector, and a simulation of the energy deposition and light transport to the detector readout electronics.

\subsection{Neutrino Beam Simulation}

To simulate the neutrino beam NOvA uses a \texttt{GEANT4} v9.2.p03~\cite{GEANT4.pdf} based Monte Carlo simulation using a detailed model of the NuMI beamline, as it was described in Sec.~\ref{sec:NOvASimulation}. The simulation starts with protons from the Main Injector interacting within the carbon based target and producing primary hadrons, followed by the transport and possible interaction of these hadrons within the focusing system, until finally ending with hadrons decaying into neutrinos producing a neutrino beam. 

This simulation is modified using the Package to Predict the Flux (PPFX) to incorporate external measurements of yields and cross sections for hadron production and interaction inside the target and the focusing horns \cite{NuMIFlux.pdf}. The current version of PPFX uses only limited results from two main experiments. For the most common $\pi$ production, PPFX uses the NA49 data \cite{NA49:Inclusive_production_of_charged_pions.pdf} from $\unit[158]{GeV/c}$ protons interacting on a thin (few percent of interaction length) carbon target. These then have to be scaled to the $\unit[20-120]{GeV/c}$ incident proton energies seen in NOvA using the FLUKA \cite{FLUKA_01,FLUKA_02} simulation. PPFX also uses Barton at al \cite{BartonHadProd1983.pdf} data for $\pi$ to extend the kinematic coverage. For $K$ production from $p+C$ interaction, important for higher neutrino energies and electron neutrinos, PPFX uses the NA49 $K$ data \cite{NA49DataKaons.pdf} and the NA49 $\pi$ data \cite{NA49:Inclusive_production_of_charged_pions.pdf} multiplied by the $K/\pi$ ratios of yields on thin carbon target from the MIPP experiment \cite{pionToKaonIn_pC.pdf}. For nucleon production PPFX uses NA49 data \cite{NA49pc-proton2013.pdf}. All the other correction, for example for nucleon incident on non carbon targets, or meson incident interactions, are either extrapolated from the previously mentioned measurements, or simply do not exist and a large systematic uncertainty is assigned to them \cite{NuMIFlux.pdf}.

I worked on including new hadron production data from the $p+C$ interaction at $\unit[31]{GeV/c}$ from the NA61 experiment \cite{2015_hadron_prod_pC_2009data.pdf}. There's a newer result for hadron production from $\unit[120]{GeV/c}$ protons on a thin carbon target \cite{NA61_hadprodFrompC_120GeV_2023.pdf}, as well as data on $p+C$ and $p+Be$ in different incident energies \cite{2019_NA61_ProdAndInelXSec_protonOnDiffTargets60And120GeV._results.pdf}, $\unit[60]{GeV/c}$ $\pi+C$ and $\pi+Be$ measurements \cite{2019_had_prod_at_Pi_on_C_and_Be.pdf}, or resonance production measurements from $\unit[120]{GeV/C}$ $p+C$ \cite{NA61_ResonanceProdFrompC_120GeV_2023.pdf}. There is also a measurements of $\pi$, $K$ and $p$ yield from the T2K experiment's replica target \cite{2019_hadron_yields_T2K_replica.pdf}, which significantly helped T2K reduce their systematic uncertainty for neutrino beam \cite{ThickTargetLimit.pdf}. Data from a similar measurement is currently being processed by the NA61 experiment for the NuMI replica target \cite{ThickTargetLimit.pdf}, hoping it would have a similar effect on NOvA's neutrino beam systematic uncertainty.

Another experiment that aims to improve the hadron production simulation is EMPHATIC~\cite{EMPHATICProposal2019.pdf}, which aims to specifically constrain nucleon production and secondary and tertiary interactions of various projectiles with a wide range of incident energies and thin target materials. This data is being currently analysed.
...
\subsection{Simulation of cosmic rays}

\note{Should also include the overlays of rock muons and cosmics and how does it work}

\subsection{Simulation of the Detector Response}

%From NOvASimulationOld-Fluka.pdf:  To shorten generation time, the modeling of detector-specific aspects, such as photon transport, detector and electronics noise, and readout electronics, employs custom, parameterized simulation applications.
...
\iffalse
GEANT4 \cite{GEANT4.pdf} simulation is used to calibrate the deposited energy of individual particles as well as the scintillation light, light transport and readout response \cite{NOvASimulationOld-Fluka.pdf}.
