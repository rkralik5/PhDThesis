...
The energy deposited within the NOvA detectors is represented by the peak \gls{ADC} value obtained from the readout electronics, as described in Sec.~\ref{sec:DAQ}.
To convert it into physical units, accounting for the attenuation of light along the \gls{WLS} fibres, or for differences between individual cell, we use calibration.
...
\todo{Describe the fibre brightness}
...
\begin{equation}
E_{dep}\ \left[\unit{MeV}\right]=\textsf{Signal}\ \left[\unit{ADC}\right]\times S\times TS_i\times R_i\left(t\right)\times A_i\left(t\right).
\end{equation}
...
\subsection{Scale}
Describe the simple ADC to PE scaling (maybe not in a separate subsection?)
...
\begin{figure}
    \centering
    \includegraphics[width=.7\textwidth]{Plots/NOvAExperiment/ExampleAttenuationFit.png}
    \caption{Example attenuation fit for a single cell in the Test Beam detector across its full physical length, shown by the dashed lines. The red line shows the initial exponential fit and the blue line shows the full fit after the LOWESS correction, both described in text.\note{Plots copied from paper}}
    \label{fig:NOvACalibrationAttenuationFit}
    %HOW TO REPRODUCE: This plot is from /nova/ana/users/rkralik/TBCalib/RelativeCalibrationResults/Atten_Fit_Results_p4.root but I manually added legend and changed size
\end{figure}
...
\begin{figure}
    ...
    \caption{To calculate the absolute energy scale we take the mean reconstructed energy response (left) for selected stopping muons for each view and each data period or simulation. We use it to divide the simulated mean true deposited energy (right).\note{Plots copied from paper}}
