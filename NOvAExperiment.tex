...
\note{The description of neutrino interactions, including QE/Res/DIS scattering and nuclear effects will probably be in the theory chapter. If not I'll add it here. I'm also counting on having defined the QE, Res, DIS, FSI and MEC abbreviations in theory.}
...
Resonant    & Berger-Sehgal \cite{BergerSehgal_ResonancePionProd_2007.pdf,BergerSehgalModel_CohPionProd_2009.pdf}          & External $\nu-A$ data\\
...
%Where are the rock events simulated? Here as well?
...
We generate cosmic rays with CRY [7] \cite{NuMIFlux.pdf} - should I describe this more?

\subsection{Simulation of the Detector Response}

To simulate the final state particles inside the NOvA detectors we use an updated version of \texttt{GEANT4} v10.4.p02~\cite{GEANT4.pdf}. This can also include creation of new particles and their propagation. This part ends with energy deposition.

Interactions in rock would be very time and resource consuming if we did them every time, therefore we do them with single particles and then overlay them on top.

The simulation of the light yield based on the deposited energy is based on a custom NOvA parametrized simulation, since every cell in NOvA is approximately the same and it would be unnecessary to simulate every cell individually \cite{NuMIFlux.pdf}. This consists of the light yield from the deposited energy, transport and attenuation of light in the fibre and the light collection in the APD. This is tuned to the cosmic data (?) - describe briefly the light level tune maybe

Another custom NOvA simulation is for the electronics, which basically just adds simulated electronics noise based on some random distributions

%NuMIFlux.pdf: The particle lists generated by either GENIE or CRY are then passed to GEANT4 [8, 9], which propagates particles through the detector and produces energy deposits in active material. Finally, the list of energy deposits in active material are passed to a parameterized front-end simulation which converts energy deposits into scintillation light, transports scintillation light to the APD, and simulates the readout electronics response. The final output is formatted like raw data.
