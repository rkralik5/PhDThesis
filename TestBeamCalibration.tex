...
We use the data-based simulation described in Sec.~\ref{sec:DataBasedSimulation}. Figure~\ref{fig:CalibhistSim} show the distribution of the hits from cosmic muon events used selected for calibration from the new simulation. We can clearly see the difference between the number of events in the vertical (even) and the horizontal (odd) planes, which is expected as cosmic muons are generally vertical and a single track often passes many more horizontal planes than vertical planes. We can also see that there are no events in cells 0 and 
