...
To validate the results of the Test Beam calibration and to check on its stability and whether there are any variations, possibly to assess the correctness of the calibration and to derive possible systematic uncertainties that would have to be applied when using these results. We use several approaches. First, we look at the same stopping muon sample that was used in the absolute calibration. As was mentioned above, these are the most trusted cosmic hits we have. We use this sample to compare the performance amongst various calibration samples (simulation and data periods and epochs), but also to verify if the calibration performs what we expect - unify the energy deposition across the detector. However, stopping muons are not very well distributed along the detector and are especially missing in the bottom of the detector, making drawing conclusions from the cell distributions difficult, especially for horizontal planes.
