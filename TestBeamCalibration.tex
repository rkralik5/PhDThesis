...
The threshold and shielding correction is intended to mitigate biases arising from the use of cosmic events instead of beam events for calibration. It is only used prior to the attenuation fits and is omitted when applying the results of the relative calibration, whether during the absolute calibration or for beam events. Presently, it is derived exclusively from simulation.

%TS correction dependence on w
We created a new threshold and shielding correction for the Test Beam detector using the new simulation described in Sec.~\ref{sec:DataBasedSimulation}. The correction is calculated for both views, across 12 \gls{FB} bins, 64 cells, and 100 bins of the position within a cell $w\in\left(\unit[-130]{cm},\unit[130]{cm}\right)$. Two examples of the correction as a function of $w$ are shown in Fig.~\ref{fig:TBThresholdCorrectionExamples}, demonstrating an almost uniform behaviour along a cell. Variations in the X view range from $\unit[1-2]{\%}$,  primarily concentrated at the edges of the cell, while the Y view exhibits sub-$\unit[1]{\%}$ variations. These trends are consistent across all Test Beam \gls{FB} bins and views. Given that the threshold and shielding correction precedes relative calibration, the absolute value of the correction is irrelevant and only the relative variations along $w$ and across cells matter.
...
The uniformity of the distributions is expected, considering the smaller size of the Test Beam detector compared to the \gls{FD}, which prompted the investigation into threshold and shielding effects. The cell length of $\unit[2.6]{m}$ has only a negligible impact on the energy distribution of cosmic muons behind the shielding correction, or on the threshold saturation, resulting in  the uniformity of the threshold and shielding correction for Test Beam detector. The larger correction at cell edges is likely caused by lower event counts in those areas. However, since this relative sparsity of events also influences relative calibration due to large variation in the energy response, the relatively larger threshold and shielding correction at cell edges is not detrimental.

%Dependence of the TS correction on plane and cell
Using the \gls{FB} map, we get a distribution of the threshold and shielding correction across Test Beam detector cells and planes, shown in the top of Fig.~\ref{fig:TBThresholdCorrectionMap}. This map demonstrates that while the correction is expected to be generally uniform across the Test Beam detector, there are notable variations between cells and planes, forming a discernible pattern. These variations and their shape primarily stem from the threshold component of the correction, shown in the bottom of Fig.~\ref{fig:TBThresholdCorrectionMap}.
...
\end{equation}
where $\gls{PE}_{Poisson \lambda}$ represents the mean of the Poisson distribution of the true deposited light $\left(\gls{PE}_{True}\right)$ and $\gls{PE}_{Reco}$ is the reconstructed number of \gls{PE} from simulation. Both $\gls{PE}_{Poisson \lambda}$ and $\gls{PE}_{True}$ are direct outputs of the light model simulation, as detailed in Sec.~\ref{sec:NOvASimulation}. After the light model simulation, $\gls{PE}_{True}$ is passed through the readout simulation, which includes a \gls{PE}-to-\gls{ADC} function for calculating the peak \gls{ADC} value. This value is then converted into $\gls{PE}_{Reco}$ using the \gls{ADC}-to-\gls{PE} scale described in Sec.~\ref{sec:NOvACalibration}. The observed shape in the threshold correction can thus be attributed to differences between $\gls{PE}_{True}$ and $\gls{PE}_{Poisson \lambda}$, as well as to various effects introduced by the readout simulation. Given the differences between $\gls{PE}_{True}$ and $\gls{PE}_{Poisson \lambda}$ are marginal (below $\unit[1]{\%}$) and contribute minimally to the threshold correction, the predominant influence on the observed pattern is exerted by the effects of the readout simulation \note{Should I somehow show another plot proving that the differences between PETrue and PEPoissionLambda are minimal, or is it enough to say?}.

%FEB versions
There are two prominent features in the threshold correction variations in Fig.~\ref{fig:TBThresholdCorrectionMap}. Firstly, the two blue vertical lines in planes 16-17 and 48-49. These planes are using the \gls{FEB} version 5.2, used in the \gls{ND}, instead of  the \gls{FEB} version 4.1, used in the \gls{FD} and in all the other Test Beam planes, as explained in Sec.~\ref{sec:TBExperiment}. While the readout simulation and the \gls{ADC}-to-\gls{PE} scale do account for for the expected disparity in the \gls{ADC} signal per \gls{PE} between the two \gls{FEB} versions, it is expected that \gls{FEB}v5.2 would exhibit a lower response to the same energy compared to \gls{FEB}v4.1. Consequently, for the same $\gls{PE}_{Poisson \lambda}$ values, the $\gls{PE}_{Reco}$ for \gls{FEB}v5.2 should be smaller than that for \gls{FEB}v4.1, resulting in a larger threshold correction in the \gls{FEB}v5.2 planes. However, the observed correction trend is contrary to this expectation, suggesting a potential error in the readout simulation regarding the handling of different \gls{FEB} versions.
...
The distribution of tricell hits from the simulated cosmic muon events selected for calibration, mapped across the Test Beam detector planes and cells, is shown in Fig.~\ref{fig:CalibhistSim}. As this is a simulated detector, we will use this `ideal conditions' distribution of tricell hits to illustrate the main features that are also present in all the data samples discussed below. We can clearly see the difference in the number of events between vertical (even) and horizontal (odd) planes. This is expected as cosmic muons are generally vertical and a single cosmic track often passes more horizontal planes than vertical planes. We can also see that due to the tricell condition there are no hits in cells 0 and 63, which are on the edge of the detector. These cells can still be calibrated by including hits from the `z tricell' condition, which is not shown in the plot. The three clear horizontal lines of relatively lower response going across the detector correspond to pairs of cells 15 + 16, 31 + 32, and 47 + 48. Together with cells 0 and 63, they represent the first and the last cell of each 16 cell-wide extrusion, which makes up half of a module, which in turn makes up half of a Test Beam plane. As was mentioned in Sec.~\ref{sec:NOvADetectors}, these cells are $\unit[3]{mm}$ narrower than the rest, resulting in fewer hits and a lower deposited energy. However, the deposited energy per path length, which is used in calibration, should compensate for this effect. Overall, Fig.~\ref{fig:CalibhistSim} demonstrates that the tricell hits are distributed fairly uniformly in the centre of the detector, with the number of hits dropping off towards the front, back and corners of the detector. This is due to the event selection applied to the cosmic tracks for calibration.
...
\begin{figure}[!hbtp]
  ...
  \end{subfigure}
  \caption[Attenuation fits for cells with readout issues in period 2 data]{Fit to the energy response in period 2. Some channels were likely dead for some time, resulting with significantly less recorded events as shown on the top left plot. This also affect their neighbouring cells due to the tricell condition as shown on the top right. Planes 55 and 57, shown on the bottom left and right plots respectively, had one of their entire \glspl{FEB} affected for some time. This results in a significantly different scale of energy response, which is much higher than the rest of the detector for plane 55, and smaller for plane 57.}
  \label{fig:AttenfitResultsPeriod2_ReadoutIssues}
\end{figure}

%Unexpected issue - Binning for the attenuation fits
An unexpected issue appeared for several cells in the end of the Test Beam detector. These cells have attenuation fits failing the calibration condition due to unusually high responses or a lack of events in the histogram bins at the edges of the cell, as shown in Fig.~\ref{fig:AttenfitResultsPeriod2_CellEdge}. This is a combination of a real physical effect - caused by fewer hits at the edge of the detector, possibly also due to the fibre loops and ends - and the choice of binning for the attenuation profiles. All attenuation profiles for all the \gls{NOvA} detectors are created with 100 bins, extending beyond the physical dimensions of the detector. For example in the Test Beam, attenuation profiles range from $\unit[-150]{cm}$ to $\unit[150]{cm}$, while the actual half-length of a Test Beam cell is $\unit[131.07]{cm}$. This means that the attenuation profile bins near the physical edges of the cells contain fewer hits from inside of the detector, resulting in larger fluctuations. Since the attenuation fits are limited to the physical cell boundaries, bins with larger variations can skew their results. This effect can be addressed either by changing the binning of the attenuation profiles to better match the physical dimension of the cells, loosening the calibration condition for hits on the edges of the cells, or using larger samples for the attenuation fits to reduce variations. Since the affected uncalibrated cells are in the end of the detector, we decided to ignore them.
...
The underfilled cells were refilled (or overfilled) during the period 3 data taking. This was the main motivation for dividing period 3 into individual epochs as shown in Tab.~\ref{tab:TestBeamPeriod3Epochs}. Another major event that could impact the Test Beam data is the replacement of several faulty \glspl{FEB}, which motivated the creation of epoch 3e.
...
The refilling of the underfilled cells can be clearly seen on the cell and plane hits distribution in Fig.~\ref{fig:CalibhistMap_period3} and on the distribution of energy deposition across horizontal (Y view) cells in Fig.~\ref{fig:Calibhist_period3}. From the distributions of hits we can also see there are a few channels that were dead for a certain time, with cell 39 in plane 48 also being affected in period 2.
From the distributions of the energy deposition we can see that during period 3 there was an \gls{FEB} that was recording a scaled up or down response, similarly to the faulty \glspl{FEB} in period 2. However, the distribution of hits shows that this particular faulty module recorded the same number of events as the surrounding modules. This is one of the \gls{FEB} that got replaced between epochs 3d and 3e. and as will be shown below this is the \gls{FEB} with the largest impact on the calibration out of the faulty \gls{FEB}s replaced before the start of epoch 3e.
...
\end{subfigure}
\caption[Plane-Cell distribution of hits for the period 3 data sample]{Distribution of events in the period 3 Test Beam data calibration sample. Comparison of Epoch 3a data before the refilling of the underfilled cells 31 and 63, clearly visible by a row of empty bins,  and a combination of Epochs 3d and 3e after the full refilling. There are also several cells that experienced readout issues, specifically cell 39 in plane 48 and cell 31 in plane 18.}
...
From the aforementioned considerations, we decided to calibrate epochs 3a, 3b and 3c together, which are all the epochs containing any underfilled cells, and to separately calibrate epochs 3d and 3e together. The faulty \gls{FEB} in the top of plane 58 is far enough in the back of the detector, that we didn't find it necessary to calibrate epochs 3d and 3e separately. Additionally, epochs 3b and 3c contain only few days worth of data, therefore they wouldn't have enough statistics for successful independent attenuation fits.

\subsubsection*{Combined Epochs 3a, 3b and 3c Relative Calibration Results}

The results of the attenuation fit for the combined epochs 3a, 3b and 3c are summarised in Fig.~\ref{fig:CellCentreResponseEp3abc}, showing the map of the fitted response at the centre of each cell. There are 182 uncalibrated cells out of 4032, constituting 4.51\% of the detector, summarised in Tab.~\ref{tab:TestBeamEp3abcRelCalibResults}.
...
The data collected during period 4 of the Test Beam run represents our best dataset, with nearly ideal detector conditions. There are a few commissioning runs in the very beginning of period 4, which uncovered some dead channels or faulty \glspl{FEB} that were immediately fixed. These initial runs constitute epoch 4a, shown on the top of Fig.~\ref{fig:CalibhistMap_period4}. Additionally, a few runs included studies where parts of the detector were masked to address \gls{FEB} saturation issues \cite{NOvA-doc-53658}, clearly visible in the middle of Fig.~\ref{fig:CalibhistMap_period4}. The bottom part of Fig.~\ref{fig:CalibhistMap_period4} shows that the remainder of the period 4 data does not have any noticeable faults in the hit distribution across the detector.
...
\end{subfigure}
\caption[Plane-Cell distribution of hits for the period 4 data sample]{Distribution of events in the Test Beam period 4 data calibration sample. The top plot shows the first three commissioning runs with readout issues, the middle plot the status of the detector during the cell masking studies and the bottom plot shows the rest of the runs. Only the runs from the bottom plot (marked GoodRuns) are used for calibration.}
\label{fig:CalibhistMap_period4}
\end{figure}

Figure~\ref{fig:Calibhist_period4} shows that the epoch 4a and the cell masking study had noticeable impact on the energy deposition across the detector. Both of these special periods only span a short time and therefore contain very limited number of hits. We decided to ignore these runs and only calibrate the rest of the period 4 data, using these results for all runs in period 4. \note{I assume here that the runs from the cell masking studies will not be used in the TB analyses. Is that correct?}
...
Results of the attenuation fits for period 4 are summarised in Fig.~\ref{fig:CellCentreResponsePeriod4} and Tab.~\ref{tab:TestBeamPeriod4RelCalibResults}. We can see that almost the entire detector is now calibrated, with only few exceptions on the edge of the detector and a single cell with an unusually high response at the edge of the cell (right plot of Fig.~\ref{fig:AttenfitResultsPeriod4}). We treated the formerly underfilled cells the same way as in epochs 3d and 3e, by manually changing the $\chi^2$ of their attenuation fits inside the csv files to be $<0.2$ and therefore making them officially calibrated. There are 108 uncalibrated cells out of 4032, totalling 2.68\% of the detector.
...
%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Validation} %Or rather Validation?
%First we look at the stopping muons which compare the periods and the simulation amongst themselves. They have some issues such as an unequal distribution across the detector, but where they exist they are the most reliable information on the calibration performance. This should also be used to check the time dependence and the stability of calibration
...
%What do we want to pay special attention to during validation? Especially checking if the things we wanted to correct for did get in fact corrected, but alsoif other things that we'd expect would've gotten corrected, did
...
In the end the performance of this calibration (and the  general performance of the NOvA calibration) will be measured by the Test Beam measurements themselves.

What we expect: the energy dependence should be uniform in space and time
...
We are only showing a selection of validation plots related to the main results and issues seen in calibration. The full suite of validation plots is included in the appendix~\ref{sec:AppTBCalibValid}.

%We look at comparions between periods for stopping muons, they might be affected and not equally distributed, but their energy deposition is very consistent
...
%Maybe I can divide this section into the individual dependencies
% 1. dependence on w (maybe calibration shape unc.?)
% 2. dependence on plane and cell (together due to the maps)
...
Also need to mention the faulty FEBs, which might be calibrated, but they're results cannot be trusted...
...
Also figuring out when were the 3 channels dead in period 2 and marking them as dead channels.