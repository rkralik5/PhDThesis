...
The threshold and shielding correction is intended to mitigate biases arising from the difference between cosmic events used for calibration and beam events. It is only used prior to the attenuation fits and is omitted when applying the results of the relative calibration, whether during the absolute calibration or for beam events. Additionally, it is derived exclusively from simulation.
...
The distribution of the threshold and shielding correction across Test Beam detector's cells and planes, shown in the top of Fig.~\ref{fig:TBThresholdCorrectionMap}, demonstrates, that while the correction is expected to be generally uniform across the Test Beam detector, there are notable variations between cells and planes, forming a discernible pattern. These variations and their shape primarily stem from the threshold component of the correction, demonstrated in the bottom of Fig.~\ref{fig:TBThresholdCorrectionMap}.
