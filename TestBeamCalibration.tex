...
The distribution of tricell hits from the simulated cosmic muon events selected for calibration as a function of the Test Beam detector planes and cells is shown in Fig.~\ref{fig:CalibhistSim}. The features on this plot illustrate the distribution of tricell hits in the `ideal conditions' Test Beam detector and are present in all the data samples discussed below as well. We can clearly see the difference in the number of events between vertical (even) and horizontal (odd) planes. This is expected as cosmic muons are generally vertical and a single cosmic track often passes more horizontal planes than vertical planes. We can also see that due to the tricell condition there are no hits in cells 0 and 63, which are on the edge of the detector. These cells can still be calibrated by including hits from the `z tricell' condition, which is not shown in the plot. The clear horizontal lines correspond to cells (0), 15, 16, 31, 32, 47, 48, (63), which are the first and the last cell of each 16 cell-wide extrusion that makes up half of a module (which makes up half of a Test Beam plane). As was mentioned in Sec.~\ref{sec:NOvADetectors}, these select cells are $\unit[3]{mm}$ narrower than the rest, resulting in fewer hits and a lower deposited energy, although a consistent deposited energy per path length. Overall, Fig.~\ref{fig:CalibhistSim} demonstrates that the tricell hits are distributed fairly uniformly in the centre of the detector, with the number of hits dropping off towards the front, back and corners of the detector. This is due to the event selection applied to the cosmic tracks for calibration.
